fn main() {
    ...
    let content: Vec<(String, usize)> = read::file(&path)
        .lines()
        .map(|line| {
...
fn part_one(movement: &[(String, usize)]) -> usize {
    let new_coordinates = movement
        .iter()
        .fold((0, 0), |coordinates, (direction, amount)| {
            let (x, y): (usize, usize) = match direction.as_ref() {
                "forward" => (coordinates.0, coordinates.1 + amount),
                "down" => (coordinates.0 + amount, coordinates.1),
                "up" => (coordinates.0 - amount, coordinates.1),
                _ => coordinates,
            };
...
fn part_two(movement: &[(String, usize)]) -> usize {
    let new_coordinates = movement
        .iter()
        .fold((0, 0, 0), |coordinates, (direction, amount)| {
            let (x, y, z): (usize, usize, usize) = match direction.as_ref() {
                "forward" => (
                    coordinates.0 + (amount * coordinates.2),
                    coordinates.1 + amount,
                    coordinates.2,
                ),
                "down" => (coordinates.0, coordinates.1, coordinates.2 + amount),
                "up" => (coordinates.0, coordinates.1, coordinates.2 - amount),
                _ => coordinates,
            };
